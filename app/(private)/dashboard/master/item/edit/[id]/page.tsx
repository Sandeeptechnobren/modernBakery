...
import { itemById, editItem } from "@/app/services/allApi";
...
export default function EditItemStepper() {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const res = await editItem(itemId, payload);
			if (res?.error) {
				showSnackbar(res.message || "Failed to update item", "error");
			} else {
				showSnackbar("Item updated successfully", "success");
				router.push("/dashboard/master/item");
			}
		} catch (err) {
			showSnackbar("Update item failed", "error");
		}
	};
						...
						<div className="p-6">
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								<div className="flex items-end gap-2 max-w-[406px]">
									<InputFields label="Item Code" name="itemCode" value={form.itemCode} onChange={handleChange} />
